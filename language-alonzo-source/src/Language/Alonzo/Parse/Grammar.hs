...
-- Declaration Rules
    ...
    dat <- rule $
      let ex (n, _) body = Data n body
      in ex <$> conId <*> (rsvp ":=" *> constrs)
...
-- Constructor Declaration Rules
    ...
    constrs <- rule $ sep (rsvp "|") constr

    constr <- rule $
      let ex (n, l) fs = Constr n (fst <$> fs)
      in  ex <$> conId <*> many varId
